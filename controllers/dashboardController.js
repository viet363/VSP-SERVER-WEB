...
export const getDashboardData = async (req, res) => {
    try {
        ...
        try {
            // Total customers
            const [[customerResult]] = await db.query("SELECT COUNT(*) as totalCustomers FROM user");
            dashboardData.totalCustomers = parseInt(customerResult?.totalCustomers) || 0;
            
            // Total products
            const [[productResult]] = await db.query("SELECT COUNT(*) as totalProducts FROM product");
            dashboardData.totalProducts = parseInt(productResult?.totalProducts) || 0;
            
            // Total orders
            const [[orderResult]] = await db.query("SELECT COUNT(*) as totalOrders FROM orders");
            ...
            // Vẫn trả về dữ liệu mặc định nếu có lỗi
        }
        ...
        // Trả về dữ liệu mặc định ngay cả khi có lỗi
        res.json({
            totalCustomers: 0,
            totalProducts: 0,
            totalOrders: 0,
